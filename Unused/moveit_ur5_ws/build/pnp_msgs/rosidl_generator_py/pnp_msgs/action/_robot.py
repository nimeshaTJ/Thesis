...
class Robot_Result(metaclass=Metaclass_Robot_Result):
    ...
    def __init__(self, **kwargs):
        assert all('_' + key in self.__slots__ for key in kwargs.keys()), \
            'Invalid arguments passed to constructor: %s' % \
            ', '.join(sorted(k for k in kwargs.keys() if '_' + k not in self.__slots__))
        self.robot_result = kwargs.get('robot_result', str())
    ...
    def __eq__(self, other):
        if not isinstance(other, self.__class__):
            return False
        if self.robot_result != other.robot_result:
            return False
        return True
    ...
    def robot_result(self):
        """Message field 'robot_result'."""
        return self._robot_result

    @robot_result.setter
    def robot_result(self, value):
        if __debug__:
            assert \
                isinstance(value, str), \
                "The 'robot_result' field must be of type 'str'"
        self._robot_result = value
    ...
    def robot_progress(self, value):
        if __debug__:
            ...
            assert not (value < -1.7976931348623157e+308 or value > 1.7976931348623157e+308) or math.isinf(value), \
                "The 'robot_progress' field must be a double in [-1.7976931348623157e+308, 1.7976931348623157e+308]"
