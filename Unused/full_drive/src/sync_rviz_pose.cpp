...
private:
    void update_robot_position() {
        ...
        try {
            ...
            if (!plan_and_visualize_trajectory(current_q)) {
                print_relaunch_message();
                return;
            }
    ...
    bool plan_and_visualize_trajectory(const std::vector<double>& joint_positions) {
        // Load robot model
    ...
    void print_relaunch_message() {
        RCLCPP_ERROR(this->get_logger(), "Sync process failed. Please relaunch the node.");
    }
